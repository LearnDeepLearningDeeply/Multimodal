...
# load dataset
dataset_train=AudioFeatureDataset(featrootdir=r'../../features/basicfbank', \
                                    cvtxtrootdir='../CV/folds',feattype='npy', \
                                    cvnum=args.cvnum,mode='train',normflag=1,\
                                    normfile=r'../../features/basicfbank/ms{}.npy'.format(args.cvnum))

dataset_eva=AudioFeatureDataset(featrootdir=r'../../features/basicfbank', \
                                    cvtxtrootdir='../CV/folds',feattype='npy', \
                                    cvnum=args.cvnum,mode='eva',normflag=0,\
                                    normfile=r'../../features/basicfbank/ms{}.npy'.format(args.cvnum))


dataset_test=AudioFeatureDataset(featrootdir=r'../../features/basicfbank', \
                                    cvtxtrootdir='../CV/folds',feattype='npy', \
...
def train(epoch,trainLoader):
    ...
    for batch_inx,(data,target,length,name) in enumerate(trainLoader):
        ...
        for i in range(batch_size):
            ...
            if(i==0):loss=F.nll_loss(torch.squeeze(output[lengthcount:lengthcount+length[i],:]),torch.squeeze(target[lengthcount:lengthcount+length[i]]))
            else:loss+=F.nll_loss(torch.squeeze(output[lengthcount:lengthcount+length[i],:]),torch.squeeze(target[lengthcount:lengthcount+length[i]]))
