...
#            nn.LogSoftmax(dim=1),
    ...
    def forward(self,x,length):
        ...
        for inx,l in enumerate(length):weight[inx,0:l]=torch.squeeze(F.softmax(potential[inx,0:l],dim=0))
...
#        print(output)       
        ...
        for i in range(batch_size):
            if(i==0):loss=loss_layer(torch.squeeze(output[i]),torch.squeeze(target[i,0]))
            else:loss+=loss_layer(torch.squeeze(output[i]),torch.squeeze(target[i,0]))
            numframes+=length[i]
...
#        print(output)
        for i in range(batch_size):
            if(batch_size==1):
                result=np.log(torch.squeeze(F.sigmoid(output[0])).cpu().data.numpy())
                test_loss+=loss_layer(torch.squeeze(output[0]),torch.squeeze(target[i,0])).data[0]
            else:
                result=np.log((torch.squeeze(F.sigmoid(output[i])).cpu().data.numpy()))
