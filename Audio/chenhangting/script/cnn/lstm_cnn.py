...
superParams_lstm={'input_dim':153,
            'hidden_dim':256,
...
'''
   ...
   ''' 
    ...
    def forward(self,x):
        ...
        out=self.cov1(x)
        out=self.cov2(out)
        out=out.view(batch_size,num_frames,self.dimAferCov)
        return out

model_lstm=LSTMNet(**superParams_lstm);model_cnn=CNNNet(**superParams_cnn)
model_lstm.cuda();model_cnn.cuda()
optimizer=optim.Adam(model.parameters(),lr=args.lr,weight_decay=0.00001)

def train(epoch,trainLoader):
    model_lstm.train();model_cnn.train()
...
#            weight=(trainLoader.dataset.ingredientWeight)[emotion_labels[label]]
            if(i==0):loss=F.nll_loss(torch.squeeze(output[i,0:length[i],:]),torch.squeeze(target[i,0:length[i]]))
            else:loss+=F.nll_loss(torch.squeeze(output[i,0:length[i],:]),torch.squeeze(target[i,0:length[i]]))
        loss.backward()
        optimizer.step()
...
def test(testLoader):
    model_lstm.eval();model_cnn.eval()
