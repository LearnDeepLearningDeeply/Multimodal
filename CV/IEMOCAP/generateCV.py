...
PATHCVDIR=r'H:/Multimodal/CV/IEMOCAP/folds'
FILTER='Ses'
        ...
        with open(PATHTXT,'r') as f:
            for line in f:
                ...
                if(len(line_)>2 and line_[0]=='[' and line_[-1]==']'):
                    ...
                    filelabel=lineSeg[2]
                    num_total2+=1
                    if(filelabel in labels):
                        if(filename[-4]=='F'):
                            filedict_F[filename]=filelabel
                        elif(filename[-4]=='M'):
                            filedict_M[filename]=filelabel
                        else:
                            sys.exit("Wrong %s"%filename)
                        num_total1+=1
    filedict[2*i0+1]=filedict_F
    filedict[2*i0+2]=filedict_M
